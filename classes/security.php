...
class Security
{
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "camerfirma_104_uggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	*  For AD it is <provider code><username>
	...
	public static function refreshDisplayName() {
		...
		if( !$fullnameField ) {
			$fullnameField = Security::usernameField();
		}
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "camerfirma_104_ugmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "camerfirma_104_uggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "camerfirma_104_ugrights", $cman->getUserGroupsConnId() );
	}
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_tickets_OwnerID", $userId );
						storageSet( "_camerfirma_104_users_gestion_OwnerID", $userId );
						storageSet( "_certificados_solicitud_OwnerID", $userId );
						storageSet( "_camerfirma_104_users_q_OwnerID", $userId );
						storageSet( "_q_group_tipotoken_chart_OwnerID", $userId );
						storageSet( "_certificados_solicitud Report_OwnerID", $userId );
						storageSet( "_dependencia_tipo_user_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["ID"] );
			storageSet( "_camerfirma_104_users_gestion_OwnerID", $data["ID"] );
						storageSet( "_certificados_solicitud_OwnerID", $data["ID"] );
						storageSet( "_camerfirma_104_users_q_OwnerID", $data["ID"] );
						storageSet( "_q_group_tipotoken_chart_OwnerID", $data["ID"] );
						storageSet( "_certificados_solicitud Report_OwnerID", $data["ID"] );
						storageSet( "_dependencia_tipo_user_OwnerID", $data["ID"] );
		}
	}
